"""Chat route — dormant until AI features are enabled.

SQLite / glossary dependency removed. Uses empty glossary list; otherwise
identical to the source implementation.
"""
import asyncio
import time
...
def _normalize_error(msg: str) -> str:
    if "not initialised" in msg or "VERTEX_AI_PROJECT" in msg:
        return "Vertex AI is not configured. Set VERTEX_AI_PROJECT in .env."
    if "PERMISSION_DENIED" in msg or "permission denied" in msg.lower():
        return "Vertex AI access denied."
    if any(k in msg for k in ("RESOURCE_EXHAUSTED", "429", "quota", "rate limit")):
        return "Vertex AI quota exceeded. Please wait and try again."
    return msg


def _fetch_widget_data(sql: str) -> list:
    cached = gemini_client._BQ_FULL_CACHE.get(sql)
    if cached and time.monotonic() - cached[0] < gemini_client._BQ_RESULT_TTL:
        return cached[1]
    return bigquery_client.run_query(sql)
...
def _build_response(result: dict) -> ChatResponse:
    ...
    if widget and widget.get("sql"):
        try:
            widget["data"] = _fetch_widget_data(widget["sql"])
