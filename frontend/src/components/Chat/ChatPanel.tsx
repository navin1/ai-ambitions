...
const STARTERS = [
  ...
  'Compare Q2 vs Q3 AI spend',
  'What drove the efficiency gain this quarter?',
  'Show vendor spend breakdown',
]
...
export function ChatPanel({ onAddWidget }: Props) {
  ...
  const [messages, setMessages]   = useState<InternalMessage[]>([
    { id: mid(), role: 'assistant', text: "Hi! I'm your AI analyst. Ask me anything about AI investments and spend data — I can answer questions, explain metrics, or generate charts." },
  ])
  ...
  useEffect(() => {
    fetch('/api/health')
      .then(r => r.json())
      .then(d => { if (d.ai_ready === false) setAiUnavailable(true) })
      .catch(() => {})
  }, [])
    ...
    try {
      ...
      for await (const event of streamChatMessage(text, buildHistory())) {
        if (event.type === 'status') {
          setMessages(prev => prev.map(m => m.loading ? { ...m, statusText: event.message } : m))
          ...
          if (msg.includes('access denied') || msg.includes('not configured')) setAiUnavailable(true)
          setMessages(prev => prev.map(m => m.loading ? { ...m, loading: false, statusText: undefined, text: msg } : m))
        }
      }
    } catch (err: unknown) {
      ...
      if (msg.includes('not configured') || msg.includes('not initialised')) {
        msg = 'AI Analyst unavailable — Vertex AI is not configured.'
        setAiUnavailable(true)
      } else if (msg.includes('quota') || msg.includes('rate limit')) {
        msg = 'AI quota exceeded. Please wait and try again.'
      }
      setMessages(prev => prev.map(m => m.loading ? { ...m, loading: false, statusText: undefined, text: msg } : m))
    } finally {
      setLoading(false)
  ...
  function toggleVoice() {
    ...
    rec.onresult = (e: SpeechRecognitionEvent) => {
      ...
      setExpanded(true); inputRef.current?.focus()
    }
