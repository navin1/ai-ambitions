...
CREATE TABLE `ai_ambitions.ai_amb_kpi_summary` (
  ...
  actual_value  FLOAT64  NOT NULL OPTIONS(description='Metric value in display units (%, pts, or raw dollars for ai-cost — see bigquery_client.py _build_tile_val)'),
...
CREATE TABLE `ai_ambitions.ai_amb_use_case_data` (
  ...
  cost_actual        FLOAT64            OPTIONS(description='Actual AI spend (raw dollars; scaled to $M in the API layer)'),
  cost_plan          FLOAT64            OPTIONS(description='Planned spend (raw dollars; scaled to $M in the API layer)'),
  ...
  revenue_actual_dollars FLOAT64            OPTIONS(description='Revenue growth contribution (raw dollars; scaled to $M in the API layer); NULL = not contributing'),
  revenue_plan_dollars   FLOAT64            OPTIONS(description='Planned revenue contribution (raw dollars; scaled to $M in the API layer)'),
